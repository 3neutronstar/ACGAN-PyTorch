...
def main():
    ...
    for epoch in range(opt.niter):
        for i, data in enumerate(dataloader, 0):
            ...
            print('\r [%d/%d][%d/%d] Loss_D: %.4f (%.4f) Loss_G: %.4f (%.4f) D(x): %.4f D(G(z)): %.4f / %.4f Acc: %.4f (%.4f)'
                % (epoch, opt.niter, i, len(dataloader),
                    errD.item(), avg_loss_D, errG.item(), avg_loss_G, D_x, D_G_z1, D_G_z2, accuracy, avg_loss_A),end='')
            if i % 100 == 0:
                ...
                fake=denormalize(fake,dataset='cifar10')
                vutils.save_image(
                    fake.data,
